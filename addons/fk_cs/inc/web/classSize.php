...
if ($operation == 'display') {
    ...
    $sid = intval($_GPC['sid']);
    if (!empty($sid)) {
        $classSize = pdo_fetch("SELECT * FROM " . tablename($this->table_classify) . " WHERE sid = '$sid'");
    } else {
        $classSize = array(
            'ssort' => 0,
        );
    ...
    if (checksubmit('submit')) {
        ...
        message('更新班级大小成功！', $this->createWebUrl($modelName, array('op' => 'display', 'schoolid' => $schoolid)), 'success');
    }
} elseif ($operation == 'delete') {
    $sid = intval($_GPC['sid']);
    $classSize = pdo_fetch("SELECT sid FROM " . tablename($this->table_classify) . " WHERE sid = '$sid'");
    if (empty($classSize)) {
