...
//		$kmname2 = $category[$teachers['km_id2']]['sname'];
        ...
        if(!empty($userid['id'])){
			$tcourses = pdo_fetchall("SELECT * FROM " . tablename($this->table_tcourse) . " WHERE :schoolid = schoolid And :weid = weid And :tid = tid ORDER BY id DESC",array(':weid' => $weid, ':schoolid' => $schoolid,':tid' => $it['tid']));
