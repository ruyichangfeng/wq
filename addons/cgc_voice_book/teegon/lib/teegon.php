...
class TeegonService {
    ...
    function verify_post_return(){
    	if($_POST['charge_id']){
    		if(empty($_POST['sign'])){
    			return array('status'=>"1",'error_msg'=>'<h1>天工服务端返回签名信息错误!</h1><hr ><pre>','param'=>$_POST);
    		}
    
    		if(!$this->get_sign_veryfy($_POST,$_POST['sign'])){
    			return array('status'=>"2",'error_msg'=>'<h1>签名验证错误请检查签名算法!</h1><hr ><pre>','param'=>$_POST);
    		}
    
    		return array('status'=>"0",'error_msg'=>'','param'=>$_POST);
    	}
    }
