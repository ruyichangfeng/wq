...
var valid_easy=function(iclass){ 
	...
	$.each(inputs,function(i,v){
		...
		if(temp==false){  
			var label=$(v).attr("label"); console.log(label);
			$.toast(label+" 不正确", "forbidden");  
...
function modalMsg(text){ 
	...
	if(height< window.screen.height-250){
	...
	if(typeof($("#popup-bg").attr("id"))=="undefined"){
		var html='<div id="popup-bg" class="popup-bg" style=" z-index: 10000000;position: absolute;top: 0px;left: 0px;width: 100%;height:'+height+'px;background-color: #000;opacity: 0.3;"></div>';		
		$(document.body).append(html);  
	}else{
		$("#popup-bg").css("display","block"); 
	}
	if(typeof($("#popup").attr("id"))=="undefined"){
		var html='<div id="popup" style="z-index:12000000;width: 30%;height: 180px;display: block;border:1px solid #ccc;border-radius: 3px;background-color:#FFF; position: absolute;top:'+top+'px;left: 50%;-webkit-transform: translate(-50%, -60%);-moz-transform: translate(-50%, -60%);-ms-transform: translate(-50%, -60%);-o-transform: translate(-50%, -60%);transform: translate(-50%, -50%);"><div style="border-bottom: 1px solid #FFFFFF;text-align: left; padding: 8px 16px;background-color:#F0F0F0;">提示';
		html+='</div><div style="text-align: center; padding: 15px;" id="pop-body">'+text+'</div><div style="width:100%;height:45px;line-height:45px;text-align:center;background-color:#F0F0F0;position:absolute;bottom:0px;"><button class="mybtn" style="margin:0 auto;cursor:pointer;width:60px;text-align:center;color:#FFF;background-color:#34A7FE;" onclick="modalMsgClose()" >关 闭</button><div></div>';
		$(document.body).append(html);  
	}else{
		$("#pop-body").html(text);
...
function modalDlg(text,callBack){ 
	var height=document.body.scrollHeight;
	if(height< window.screen.height-250){
		height=window.screen.height-290;
	} 
	...
	if(typeof($("#sssmodal-bg").attr("id"))=="undefined"){
		var html='<div id="modal-bg" class="popup-bg" style=" z-index: 10000000;position: absolute;top: 0px;left: 0px;width: 100%;height:'+height+'px;background-color: #000;opacity: 0.3;" ></div>';		
		$(document.body).append(html);  
	}else{
		$("#sssmodal-bg").css("display","block"); 
	}
	if(typeof($("#sssmodal").attr("id"))=="undefined"){
		var html='<div id="sssmodal" style="z-index:12000000; min-width:500px;display: block;border:1px solid #ccc;border-radius: 3px;background-color:#FFF; position: absolute;top: '+top+'px;left: 50%;-webkit-transform: translate(-50%, -60%);-moz-transform: translate(-50%, -60%);-ms-transform: translate(-50%, -60%);-o-transform: translate(-50%, -60%);transform: translate(-50%, -50%);"><div style="border-bottom: 1px solid #FFFFFF;text-align: left; padding: 8px 16px;background-color:#F0F0F0;">提示';
		html+='</div><div style="text-align: center;padding: 15px;padding-bottom:30px; " id="pop-body2">'+text+'</div><div style="width:100%;height:50px;line-height:45px;text-align:center;background-color:#F0F0F0;position:absolute;"> ';
		html+='<button  onclick="'+func+'" style="margin:10px 40px 10px 143px;height:30px;padding:4px 8px;border-raduis:5px;line-height:1.4;float:left;cursor:pointer;border:1px solid transparent;width:60px;text-align:center;color:#FFF;background-color:#34A7FE;"  >确 认</button>';
		html+='<button class="mybtn" style="margin:10px 143px 10px 40px;height:30px;padding:4px 8px;border-raduis:5px;line-height:1.4;float:left;cursor:pointer;border:1px solid #999;width:60px;text-align:center;background-color:#FFF;color:black;" onclick="modalDlgClose()" >关 闭</button><div></div>';
		$(document.body).append(html);  
	}else{
		$("#pop-body2").html(text);
