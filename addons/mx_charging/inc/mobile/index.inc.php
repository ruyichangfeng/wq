...
if($op == 'index') {
	...
	$agent_truename = $user_info['true_name'];
	
	//获取设备统计信息
	$device_total = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('xc_device')." where uniacid=".$_W['uniacid']." and aid=".$_GPC['aid'].""); //设备总数
	
	
	$device_total_ok = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('xc_device')." where uniacid=".$_W['uniacid']." and aid=".$_GPC['aid']." and status=1"); //正常设备
	
	//$tt="SELECT COUNT(*) FROM ".tablename('xunchi90_device')." where status=1"; //
	$device_total_er = $device_total - $device_total_ok; //离线
	
	//扫码支付
	$scan_total = pdo_fetchcolumn("SELECT SUM(pay_money) FROM ".tablename('xc_financial')." where uniacid=".$_W['uniacid']." and pay_type=2 and  status=1 and aid ='".AID ."' and appid='".APPID ."'");	
	
	//余额支付
	$money_total = pdo_fetchcolumn("SELECT SUM(pay_money) FROM ".tablename('xc_financial')." where uniacid=".$_W['uniacid']." and pay_type=15 and  status=1 and aid='".AID ."' and appid='".APPID ."'");	
	
	//充值总额
	$inser_total = pdo_fetchcolumn("SELECT SUM(pay_money) FROM ".tablename('xc_financial')." where uniacid=".$_W['uniacid']." and pay_type=6 and  status=1 and aid='".AID ."' and appid='".APPID ."'");	
