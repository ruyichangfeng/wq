...
if ($op == 'display'){
	if (!empty($id)){
		...
		if ($mem['openid']==$_W['openid'] && !empty($mem['openid'])){
			...
			if ($cfg['auth'] && !$mem['authopenid']){
				if (!empty($_GPC['code'])){
					...
					$_SESSION['daily_auth_openid'.$_W['uniacid']] = $oauthopenid = $res['openid'];
			...
			}else $_SESSION['daily_auth_openid'.$_W['uniacid']] = $mem['authopenid'];
		}else{
			if (strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') === false) {
					$iswx = false;
			}else{
				if ($cfg['auth'] && empty($_SESSION['daily_auth_openid'.$_W['uniacid']])){
					if (!empty($_GPC['code'])){
						load()->func('communication');
						$cfg['appid'] = trim($cfg['appid']);
						$cfg['appsecret'] = trim($cfg['appsecret']);
						...
						$res = ihttp_get($url);
						$res = @json_decode($res['content'],true);
						$_SESSION['daily_auth_openid'.$_W['uniacid']] = $res['openid'];
					}elseif (!empty($cfg['appid'])){
						$callback = urlencode($_W['siteroot']."app/".$this->createMobileUrl('myWorks',array('wid'=>$_GPC['wid']),true));
						$url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid={$cfg['appid']}&redirect_uri={$callback}&response_type=code&scope=snsapi_base&state=1#wechat_redirect";
						header('location:'.$url);
						exit;
		...
		if (!empty($works['imgs'])){
			$hasratio = true;
			foreach ($works['imgs'] as $k =>$value){
				//图片没有宽高比则补上
				if (empty($value['ratio'])){
					$hasratio = false;
					$im = getimagesize($value['img']);
					$ratio = $im[0]/$im[1];
					$works['imgs'][$k]['ratio'] = $ratio;
				}
				
				if (!empty($value['img'])){
					if ($cfg['isqiniu']){
						if (strpos($value['img'],'?') === false) {
							$works[$k]['img'] .= "?imageView2/2/w/420/interlace/0/q/60";
						}else $works[$k]['img'] .= "/imageView2/2/w/420/interlace/0/q/60";
					}
				}
			}
			if (!$hasratio){
				pdo_update($this->modulename.'_works',array('imgs'=>serialize($works['imgs'])),array('id'=>$works['id']));
			}
		}
		...
		$thumb = $works['imgs'][0]['img'];
		if (strpos($works['imgs'][0]['img'], 'http://') === false) $thumb = toimage($works['imgs'][0]['img']);
	...
	if ($cfg['auth'] && empty($_SESSION['daily_auth_openid'.$_W['uniacid']])){
		if (!empty($_GPC['code'])){
			...
			$callback = urlencode($_W['siteroot']."app/".$this->createMobileUrl('myWorks',array('wid'=>$_GPC['wid'],'op'=>'edit'),true));
			$url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid={$cfg['appid']}&redirect_uri={$callback}&response_type=code&scope=snsapi_base&state=1#wechat_redirect";
			header('location:'.$url);
			exit;
		}
