...
if(isset($post['_fm']) && $post['_fm']=='index'){
    ...
    if($isInsert && $res){
        ...
        $mMsg=array();
        $sMsg=array();
        $url = $_W['siteroot']."/".$this->createMobileUrl('mdetail',array("id"=>$osn,'app_id'=>$appInfo['id']));
        //支付暂时存储模板消息
        if(isset($orderId)){
        ...
        $url1 = $_W['siteroot']."/".$this->createMobileUrl('detail',array("id"=>$osn,'app_id'=>$appInfo['id']));
    ...
    if(isset($post['photo'])){
        $photo =json_decode($post['photo']['url'],true);
        $urlList = $this->media($photo,true);
        $post['photo']['url']=json_encode($urlList);
    }
    if(isset($post['voice'])){
        $voice = $this->media($post['voice']['url'],false);
        $post['voice']['url']=$voice;
    }
    //解析数据
    ...
    $sMsg=array();
    $mMsg=array();
    $url1 = $_W['siteroot']."/".$this->createMobileUrl('detail',array("id"=>$post['recorder_id'],'app_id'=>$appInfo['id']));
    if($nextNode['who']==1){
        if($appInfo['mods']==0) {
    ...
    $url = $_W['siteroot']."/".$this->createMobileUrl('mdetail',array("id"=>$post['recorder_id'],'app_id'=>$appInfo['id']));
    if(!empty($memberInfo)){
        if(isset($orderId)){
            $item=array();
