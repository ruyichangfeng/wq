...
class advs_log
{
    public $table = 'imeepos_coach_advs_log';
    ...
    public function check($openid){
        global $_W;
        $item = $this->get(array('openid'=>$openid,'uniacid'=>$_W['uniacid']));
        if(empty($item)){
            return true;
        }else{
            return false;
    ...
    public function get($params){
        return pdo_get($this->table,$params);
    }
    ...
    public function install(){
        ...
        if(!pdo_fieldexists($this->table,'adv_id')){
            pdo_query("ALTER TABLE ".tablename($this->table)." ADD COLUMN `adv_id` int(11) DEFAULT '0'");
        }
