...
$urlt = $this->createWebUrl('staff_class');
...
$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('cwgl_staff_class') . " WHERE weid = '{$_W['uniacid']}' and pid=0 and type = 'zw'");
if($total){
	$list = pdo_fetchall("SELECT * FROM " . tablename('cwgl_staff_class') . " WHERE weid = '{$_W['uniacid']}' and pid=0 and type = 'zw' ORDER BY listorder ASC, cid DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
	$pager = pagination($total, $pindex, $psize);
}

include $this->template('staff_class');
?>