...
<blockquote class="layui-elem-quote">
    <div class="layui-inline">
        <div class="layui-input-inline">
            <input type="text" value="<?php  echo $_GPC['name'];?>" id="name" placeholder="名称搜索" class="layui-input search_input">
        </div>
        <a class="layui-btn" lay-submit lay-filter="search">搜索</a>
    </div>
    <?php  if($barMenu['add']) { ?>
    <div class="layui-inline">
        <a class="layui-btn layui-btn-normal newsAdd_btn" data-url="<?php  echo $this->createWebUrl('Add',array('tb'=>'flow'))?>&tp=add">添加流程</a>
    </div>
    <?php  } ?>
...
<div class="layui-form news_list">
    <table class="layui-table">
        ...
        <tr>
            ...
            <td>
                <?php  if($barMenu['edit']) { ?>
                <a class="layui-btn layui-btn-mini news_edit" data-type="edit" data-id="<?php  echo $admin['id'];?>">
                    <i class="iconfont icon-edit"></i>编辑
                </a>
                <?php  } ?>
                <?php  if($barMenu['desi']) { ?>
                <a class="layui-btn layui-btn-mini layui-btn-danger ceshi"  data-type="des" data-id="<?php  echo $admin['id'];?>">
                    设计流程(拖拽)
                </a>
                <!--<a class="layui-btn layui-btn-mini news_edit"  data-type="des" data-id="<?php  echo $admin['id'];?>">-->
                    <!--设计流程(表单)-->
                <!--</a>-->
                <?php  } ?>
                <?php  if($barMenu['delete']) { ?>
...
<script>
    layui.use(['form','jquery','laypage'], function(){
        ...
        form.on('submit(search)', function(data){
            name=$("#name").val();
            mobile=$("#mobile").val();
            url = "<?php  echo $this->createWebUrl('list',array('tb'=>'flow'))?>"+"&name="+name;
            location.href=url;
            return false;
        });
        ...
        setting.jump=function(obj, first){
            ...
            mobile=$("#mobile").val();
            if(first!=true){
                url = "<?php  echo $this->createWebUrl('list',array('tb'=>'flow'))?>"+"&name="+name+"&page="+obj.curr;
        ...
        $(".news_del").on("click",function(){
            ...
            layer.confirm('确定删除此信息？',{icon:3, title:'提示信息'},function(index){
                ...
                $.post("<?php  echo $this->createWebUrl('delete')?>&tb=flow&id="+id,function(response){
        ...
        $(".news_edit").on("click",function(){
            ...
            var url = "<?php  echo $this->createWebUrl('editFlow')?>&id="+id+"&tp="+type;
        ...
        $(".ceshi").on("click",function(){
            var id = $(this).attr("data-id");
            var type =$(this).attr("data-type");
            var url = "<?php  echo $this->createWebUrl('desFlow')?>&id="+id+"&tp="+type;
            var index = layui.layer.open({
                title : "设计流程",
                type : 2,
                content : url,
