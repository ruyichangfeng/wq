...
// );       
	if(checksubmit('submit')){
		...
		if($row){
			message('添加幻灯片成功！',$this->createWebUrl('nav2'),array('op'=>'add'),'success');
		}else{
			message('添加幻灯片失败！',$this->createWebUrl('nav2'),array('op'=>'add'),'error');
			}
		}
	}
if( $op=='update'){
	...
	if (!empty($resut)) {
			message('图片修改成功！', $this->createWebUrl('nav2', array('op'=>'display')), 'success');
		} else {
			message('图片修改失败！');
		}
	}
...
if($op=='delete'){
	$id = intval($_GPC['id']);
			...
			if(intval($result) == 1){
				message('删除图片成功！', $this->createWebUrl('nav2'), 'success');
			} else {
				message('删除图片失败！');
			}
}
...
include $this->template('web/nav2');
