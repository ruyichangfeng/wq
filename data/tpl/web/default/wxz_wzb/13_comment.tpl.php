	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			<table class="table table-hover">
				...
				<tbody>
				<?php  if(is_array($Comment)) { foreach($Comment as $row) { ?>
				<?php  if($row['dsid']==0 || ($row['dsid']>0 && $row['dsstatus']==1)) { ?>
				<tr>
					<td><input class="check-delete tagids-<?php  echo $row['id'];?>" type="checkbox" name="delete[]" value="<?php  echo $row['id'];?>" data-tagids="<?php  echo $row['groupid'];?>" data-openid="<?php  echo $row['openid'];?>" data-fanid="<?php  echo $row['id'];?>"/></td>
					...
					<td align="centet" style="text-align: center;">
						<?php  if($row['is_auth']==1) { ?>
						<span style="color: green;">显示</span><?php  } ?>
						<?php  if($row['is_auth']==2) { ?>
						<span style="color: red;">不显示</span>
						<?php  } ?>
					</td>
					<td align="centet" style="text-align: center;">
						<?php  if($row['ispacket']==1) { ?>
						<span style="color: green;">是</span><?php  } ?>
						<?php  if($row['ispacket']==0) { ?>
					...
					<td align="center" style="text-align:center;"><?php  echo $row['send_num'];?></td>
					...
					<td align="center" style=" text-align:center;">
						<?php  if($row['is_auth']==1) { ?>
						<a onclick="auth(<?php  echo $row['id'];?>,2,<?php  echo $row['rid'];?>);"><button type="button" class="btn btn-warning" >不显示</button></a>
						...
						<?php  } ?>
						<?php  if($row['is_auth']==2) { ?>
						<a onclick="auth(<?php  echo $row['id'];?>,1,<?php  echo $row['rid'];?>);"><button type="button" class="btn btn-success" >显示</button></a>
						<?php  } ?>
						<?php  if($row['isshutup']==0) { ?>
						...
						<?php  } ?>
						<?php  if($row['ispacket']==1 && $row['isadmin']==1) { ?>
						<a href="<?php  echo $this->createWebUrl('getpacket',array('toid' =>$row['id'],'is_auth'=>'1','rid'=>$row['rid']))?>"><button type="button" class="btn btn-success" >领取用户</button></a><?php  } ?>
						<?php  if($row['ispacket']!=1 && $row['isadmin']!=1 && $row['touid']==0 && $row['dsid']==0) { ?><a href="<?php  echo $this->createWebUrl('reply',array('id' =>$row['id'],'rid'=>$row['rid']))?>"><button type="button" class="btn btn-success" >回复</button></a><?php  } ?>
					</td>
				</tr>
				<?php  } ?>
				<?php  } } ?>
