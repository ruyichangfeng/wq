...
LIMIT 0 , 30
 */
//类型 1：关注闲书币  2：每日闲书币  3：连续闲书币 4、系统奖励
class Mon_SignModuleSite extends WeModuleSite
{
    ...
    public function  doWebSystemaward()
    {
        ...
        if (checksubmit()) {
            ...
            if ($sign['sync_credit'] == 1) {//同步闲书币
                $this->synFanscredit($signUser['openid'],$_GPC['credit']);//同步
            }

            CRUD::updateById(CRUD::$table_sign_user, $user_data, $uid);// 更新用户闲书币
    ...
    public function doMobileSign()
    {
        ...
        if (empty($signUser)) {//从来没有签过到用户处理
            ...
            CRUD::create(CRUD::$table_sign_award, $sign_award_day);// 插入闲书币记录
            $res['code'] = 200;
            $res['msg'] = "恭喜您获得日签到闲书币" . $sign['sign_credit'] . "分!";

            if ($sign['sync_credit'] == 1) {//同步闲书币
                $this->synFanscredit($clientUser['openid'], $sign['sign_credit']);
            }
            //同步闲书信息
            $this->processXs($sign['sign_credit'],$clientUser['openid']);
            echo json_encode($res);
            exit;
        ...
        if (!empty($signUser)) {
            ...
            CRUD::create(CRUD::$table_sign_award, $sign_award_day);// 插入日闲书币得奖表
            ...
            if ($recordOffsetDays < $offsertDays) {// 签到时间断了，重新计算
                ...
                CRUD::create(CRUD::$table_sign_award, $sign_award_serial);//  插入连续闲书币
                //更新award 表
            ...
            CRUD::updateById(CRUD::$table_sign_user, $m_sin_user_data, $signUser['id']);//更新用户的闲书币

            if ($sign['sync_credit'] == 1) {//同步闲书币
                $this->synFanscredit($clientUser['openid'],$mserial_credit+$sign['sign_credit']);
            }
            $res['code'] = 200;
            $ser_msg = $mserial_credit > 0 ? "连续签到闲书币" . $mserial_credit . "分" : "";
            $res['msg'] = "恭喜您获日签到闲书币" . $sign['sign_credit'] . "分" . $ser_msg;
            //同步闲书信息
            $this->processXs($sign['sign_credit'],$clientUser['openid']);
            echo json_encode($res);
            exit;
...
//函数==========================================================================================================
    ...
    public function  synFanscredit($openid, $credit)
    {
        ...
        if(!empty($fan)){
            mc_credit_update($fan['uid'],'credit1',$credit, array($_W['uid'],'闲书币签到增加闲书币'));
        }
    ...
    public function processXs($sign_credit,$openid){
        global $_W,$_GPC;
        if(!empty($sign_credit)){
            //开始处理额度
            $userAccount = pdo_fetch("SELECT * FROM " . tablename('wx_school_bookaccount') . " where openid = :openid", array(':openid' => $openid));
            if(!empty($userAccount)){
                $sign_amount = $userAccount['sign_amount'] + $sign_credit;
                $balance = $userAccount['balance'] + $sign_credit;
                pdo_update('wx_school_bookaccount',array('sign_amount' => $sign_amount,'balance' => $balance),array('openid' => $openid));
                //发送模板消息
                $balance1 = $userAccount['balance'];
                $orders = pdo_fetch("SELECT sum(price) as orderPrice FROM " . tablename('wx_school_myorder') . " where openid = :openid and orderStatus in (1,2,3)", array(':openid' => $openid));
                if($orders['orderPrice']){
                    $balance1 -= $orders['orderPrice'];
                }
                $trans_orders = pdo_fetchall("SELECT inPrice,outPrice,openid,partybopenid FROM " . tablename('wx_school_mytransferorder') . " where  orderStatus in (1,2,3,4,5) And (openid = '{$openid}' or partybopenid = '{$openid}')");
                ...
                $msgBody = array(
                    'schoolid'   => $_GPC['schoolid'] ? $_GPC['schoolid'] : 8,
                    'weid'       => $_W['uniacid'],
                    'orderId'    => 10000,
                    'orderType'  => 'sign',
                    'openid'     => $openid,
                    'amountData' => array(
                        'preAmount'   => $balance1,
                        'afterAmount' => $balance1+$sign_credit,
                        'amount'      => $sign_credit
                    )
                );
