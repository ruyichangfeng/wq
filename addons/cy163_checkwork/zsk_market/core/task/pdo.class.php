...
class Model
{
	...
	private $fetchcolumnSts=0;
	function __construct($name){  
		global $config; 
		if(trim($config['db']['host'])>0){//商业版
			$this->db_prefix=$config['db']['tablepre'];
		}else{
			$this->db_prefix=$config['db']['master']['tablepre'];
		}  
		$this->tab=$this->prefix."_".$name;
		$this->table=$this->db_prefix.($this->prefix."_".$name);
		$this->__connection();
	...
	public function table($tab){
		global $config;  
		$this->table=$this->db_prefix.($tab);
		return $this;
	}
	function reset(){
		$this->fetchcolumnSts=-1;
		$this->wherestr="";
		$this->orderstr="";
		$this->limitstr="";
		$this->groupstr="";
	...
	function tablename($tab){
		$table=$this->db_prefix.($this->prefix."_".$tab); 
		return "`".$table."`";
	}
	function tablemodel($tab){ 
		$this->table=$this->db_prefix.($this->prefix."_".$tab); 
		return $this;
	}
	...
	function __connection(){
		global $config; 
		try {   
			if(strlen(trim($config['db']['host']))>2){//商业版
				$dns="mysql:dbname=".$config['db']['database'].";host=".$config['db']['host'].";charset=utf8";
				$this->connect = new PDO($dns, $config['db']['username'], $config['db']['password']);
			}else{
				$dns="mysql:dbname=".$config['db']['master']['database'].";host=".$config['db']['master']['host'].";charset=utf8";
				$this->connect = new PDO($dns, $config['db']['master']['username'], $config['db']['master']['password']);
			}  
	...
	function execute($sql,$data=array()){
		...
		$type2=strtoupper(substr(trim($sql),0,4));
		$pre=$this->connect->prepare($sql); 
		$pre->execute($data);
		$stats="true";
		switch ($type2) {
			case 'SHOW':
		...
		switch ($type) {
			case 'INSERT':
				$result=intval($this->connect->lastInsertId());
				break;
			case 'SELECT':  
				if(intval($this->limitstr)==1){ 
					$result = $pre->fetch(PDO::FETCH_ASSOC); 
					 
					if(intval($this->fetchcolumnSts)==1){
						 
						if(count(explode(",",$this->fields))==1&&(trim($this->fields)!="*")){
							$result=$result[trim($this->fields)];
						}
					}else if(intval($this->fetchcolumnSts)==2){
				...
				break;
			case 'UPDATE':
				$result=intval($pre->rowCount());
				break;
			case 'DELETE':
				$result=intval($pre->rowCount());
				break; 
			default:
				$result="NOT MACTH SQL TYPE!";
				break;
		}
		}
		$this->reset();
