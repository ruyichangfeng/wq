...
class Duola_clubModuleSite extends Core {

    public function __construct()
    {
        $this->modulename = 'duola_club';
    }
	...
	public function doWebBook_lb() {
        $this->processWebBook(strtolower(substr(__FUNCTION__,5)),true);
	}
	...
	public function doWebBook_distance() {
        $this->processWebBook(strtolower(substr(__FUNCTION__,5)));
	}
    ...
    public function doWebBook_min_margin() {
        $this->processWebBook(strtolower(substr(__FUNCTION__,5)));
    }
    ...
    public function doWebBook_cancel(){
	    $this->processWebBook(strtolower(substr(__FUNCTION__,5)));
    }
    public function doWebBook_discount(){
        $this->processWebBook(strtolower(substr(__FUNCTION__,5)));
    }
    public function doWebBook_user(){
        $this->getLogic ( __FUNCTION__, 'web' );
    }
	// ====================== FUNC =====================
...
//		$this->getLogicmb ( __FUNCTION__, 'mobile', true );
        ...
        $this->doMobileDispatchMobileBook();
	}
...
//		$this->getLogicmb ( __FUNCTION__, 'mobile', true );
        //检查用户是否注册
        global $_GPC,$_W;
        $user = pdo_fetch("SELECT * FROM " . tablename($this->table_user) . " where :schoolid = schoolid And :weid = weid And :openid = openid And (sid > 0 Or tid > 0 Or pid > 0) order by createtime", array(':weid' => $_W['uniacid'], ':schoolid' => $_GPC['schoolid'], ':openid' => $_W['openid']), 'id');
        if(empty($user)){
            ...
            echo "<script type='text/javascript'>alert('请先完善信息!');</script>";
            include $this->template('bangding');
        }else{
            if(empty($_GPC['op'])){
                $_GPC['op'] = 'displayUnSaleBook';
            }
    ...
    public function doMobileScancode(){
        global $_GPC;
        $_GPC['op'] = 'scanCode';
        $this->doMobileDispatchMobileBook();
    }
   ...
   //处理web闲书业务
    private function processWebBook($functionName,$tableName = null){
        checkLogin ();//检查登录情况
        $this->dispatchWebBook($functionName,$tableName);
    }

    //处理mobile闲书业务
    private function processMobileBook(){
        global $_GPC,$_W;
        $schoolid = $_GPC['schoolid'];
        $weid = $_W['uniacid'];
        $openid = $_W['openid'];
        $op = $_GPC['op'] ? $_GPC['op'] : 'borrow_in' ;
        $orderType    = $_GPC['orderType'] ? $_GPC['orderType'] : 'borrow';
        if(isset($_GPC['is_accept_exchange'])){
            if($_GPC['is_accept_exchange'] == 1){
                $orderType = 'borrow';
            }else{
                $orderType = 'transfer';
            }
        ...
        switch ($op){
            ...
            case 'confirmOrder':
                ...
                break;
            case 'cancelBook':
                $functionName = 'cancelBook';
                break;
            case 'borrow_out':
                $functionName = 'displayBorrowOut';
                break;
            case 'borrow_in':
                $functionName = 'displayBorrowIn';
                break;
            case 'transfer':
                $orderType = 'transfer';
                $functionName = 'displayTransfer';
                break;
            default:
                $functionName = 'display';
                break;
        }
        include_once 'inc/func/isauth.php';//处理权限
        if($orderType == 'borrow'){
            include_once 'inc/mobile/book/MobileBookController.php';
            $bookController = new MobileBookController($schoolid,$openid,$weid);
        }else{
            include_once 'inc/mobile/book/MobileTransferBookController.php';
            $bookController = new MobileTransferBookController($schoolid,$openid,$weid);
        }
        return $bookController->$functionName();
    }

    public function doMobileDispatchMobileBook(){
        global $_GPC,$_W;
        $schoolid = $_GPC['schoolid'];
        $weid = $_W['uniacid'];
        $openid = $_W['openid'];
        $op = $_GPC['op'] ? $_GPC['op'] : '' ;
        if(!empty($op)){
            include_once 'inc/mobile/book/BaseBookController.php';
            $baseBook = new BaseBookController($schoolid,$openid,$weid);
            $data = $baseBook->$op();
            if(!empty($data['data']['template'])){
                $school = $data['school'];
                include $this->template($data['data']['template']);
            }
        }
    }

    public function doWebDispatchWebBook(){
        global $_GPC,$_W;
        $schoolid = $_GPC['schoolid'];
        $weid = $_W['uniacid'];
        $openid = $_W['openid'];
        $op = $_GPC['op'] ? $_GPC['op'] : '' ;
        if(!empty($op)){
            include_once 'inc/mobile/book/WebBookController.php.php';
            $baseBook = new BaseBookController($schoolid,$openid,$weid);
            $data = $baseBook->$op();
            if(!empty($data['data']['template'])){
                $school = $data['school'];
     ...
     * 处理modelName的逻辑
     */
    private function dispatchWebBook($modelName,$tableName = null){
        global $_GPC,$_W;
        ...
        if ($operation == 'display') {
            ...
            if (!empty($sid)) {
                if($tableName == null){
                    $book_data = pdo_fetch("SELECT * FROM " . tablename($this->table_bookmargin) . " WHERE id = '$sid'");
                }else{
                    $book_data = pdo_fetch("SELECT * FROM " . tablename($this->table_classify) . " WHERE sid = '$sid'");
                }
            } else {
                $book_data = array(
                    'ssort' => 0,
            ...
            if (checksubmit('submit')) {
                ...
                if($tableName == null){
                    $data = array(
                        ...
                        'couponAmount' => $_GPC['couponAmount'] > 0 ? $_GPC['couponAmount'] : 0,
                        'type' => $modelName,
            ...
            $sid = intval($_GPC['id']);
            if($tableName == null) {
                $courseCats = pdo_fetch("SELECT id FROM " . tablename($this->table_bookmargin) . " WHERE id = '$sid'");
            }else{
...
//        'theclass' => '班级管理',
    ...
    function uploadFile($file, $filetempname, $array) {
        //自己设置的上传文件存放路径
        $filePath = '../addons/duola_club/public/upload/';
	...
	public function payResult($params) {
		...
		if ($params['from'] == 'return') {		
			if($order['type'] == 4){		
				$url = $_W['siteroot'] . 'app/index.php?i=' . $wxpay['weid'] . '&c=entry&schoolid=' . $wxpay['schoolid'] . '&id=' . $order['signid'] . '&do=signupjc&m=duola_club';
			}else if($order['type'] == 5){		
				$url = $_W['siteroot'] . 'app/index.php?i=' . $wxpay['weid'] . '&c=entry&schoolid=' . $wxpay['schoolid'] . '&do=user&m=duola_club';
			}else{
				$url = $_W['siteroot'] . 'app/index.php?i=' . $wxpay['weid'] . '&c=entry&schoolid=' . $wxpay['schoolid'] . '&do=user&m=duola_club';
			}
			...
			if($hits > 0 && $hits >= $margin['amount']){
				...
				if($account1['jy_amount'] == null || $account1['jy_amount'] == '0.00'){
					...
					$msgBody = array(
						...
						'amountData' => array(
							'preAmount'   => $balance1,
							'afterAmount' => $balance1+$margin['couponAmount'],
