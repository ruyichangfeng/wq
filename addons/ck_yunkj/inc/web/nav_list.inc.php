...
$urlt = $this->createWebUrl('nav_list');
...
if(checksubmit('add_submit') || checksubmit('edit_submit')){
	...
	if(checksubmit('add_submit')){
		 ...
		 pdo_insert('cwgl_nav_list', $data);
		 message('添加成功', $urlt, 'success');
	}
	
	//修改
	if(checksubmit('edit_submit')){
		pdo_update('cwgl_nav_list', $data, array('id' => $_GPC['idd']));
...
if($op == 'edit'){
	$id = intval($_GPC['id']);
	$srdb = pdo_get('cwgl_nav_list', array('id' => $id));
	if (empty($srdb)) {
		message('不存在或是已经被删除！');
	}
...
if($op == 'delete'){
	$id = intval($_GPC['id']);
	if($id){
		pdo_delete('cwgl_nav_list', array('id' => $id));
		...
		switch ($_GPC['optype']) {
			case '1':
				...
				for($i=0;$i < count($ids); $i++){
					pdo_delete('cwgl_nav_list', array('id' => $ids[$i]));
				}
...
$list = pdo_fetchall("SELECT * FROM " . tablename('cwgl_nav_list') . " WHERE weid = '{$_W['uniacid']}' ORDER BY listorder ASC,id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('cwgl_nav_list') . " WHERE weid = '{$_W['uniacid']}'");
