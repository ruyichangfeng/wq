		...
		mounted: function () {
			...
			$.ajax({
				...
				success: function (data) {
                    ...
                    //计划任务
                    that.cronUrl = './index.php?i='+ uniacid +'&lid='+ lid +'&c=entry&do=ftpuploadcron&m=wxz_piclive';
				},
...
//			  	},10);
            	...
            	$.ajax({
                        ...
                        success: function (res) {
                            if(res.err == 0){
								...
								app.imageList[index].currentSize = app.imageList[index].bigImgSize * 1024;
								console.log(app.imageList[index].bigImgSize);
                            }
                        }
                })
			...
			toFindMe: function() {
				this.showMe = true;
				document.getElementById('component-pop-me').style.display = 'block';
			},
			findmeUploadBtn: function() {
				
				var imgurl = document.getElementById('meImgUrl').value;
				var that = this;
				$.ajax({
					url: './index.php?i='+ uniacid +'&c=entry&do=addimage&op=findme&m=wxz_piclive',
					type: 'POST',
					data:{meImgUrl: imgurl},
					dataType:'json', 
					beforeSend: function () {
						console.log('加载中');
					    $("#zhezhao").css('display', 'block').css('opacity', '0.5');
					},
					success: function (res) {
						that.fixed_top = document.body.scrollTop || document.documentElement.scrollTop;
						$('body').css({'position': 'fixed', 'top': -that.fixed_top + 'px' });
						$("#zhezhao").css('display', 'none');
						if(res.err > 0){
							alert(res.msg);return false;
						}
						that.showMeList = true;
						that.imageList = [];
						that.imageList = res.findedImg['list'];
						that.meImgCount = res.findedImg['count'];
		    ...
		    fileMeAdd(file){
		        ...
		        reader.onload = function () {
		          ...
		          //赋值
		          $.ajax({
						url: './index.php?i='+ uniacid +'&c=entry&do=addimage&op=upload&m=wxz_piclive',
						type: 'POST',
						data:{filedata:this.vue.imgMeList[0].file.src},
						dataType:'json', 
						beforeSend: function () {
							console.log('加载中111');
							$("#zhezhao").css('display', 'block').css('opacity', '0.5');
						...
						success: function (res) {
							...
							$("#zhezhao").css('display', 'none');
							this.meImgUrl = res.imgUrl;
							document.getElementById('meImgUrl').value = res.imgUrl;
						}
					});
		        }
		        //上传图片
		        document.getElementById('findMeBtn').style.display = 'block';
		        console.log(document.getElementById('album-upload-btn-text').innerHTML);
		        document.getElementById('album-upload-btn-text').innerHTML = '重新上传';
		    },
		}
	});
});