...
class Wechat{
    ...
    public $appSecret; 
    public $uniacid;
    public function __construct($uniacid=0,$appid="",$secret='')
    {
    	global $wxapp;
        $db=new Model("token");
        if($uniacid>0){
              $data0=$db->table("account_wechats")->where("uniacid=".$uniacid."")->get("`key`,secret");
              $this->uniacid=$uniacid;
              $this->appId = $data0['key'];
              $this->appSecret =$data0['secret'];
        }else{ 
              $this->uniacid=$appid;
              $this->appId = $appid;
    ...
    public function getToken($new=false){//快捷方法
        if($new){
            ...
            global $wxapp;
	        $db=new Model("token");
	        $data0=$db->table("core_cache")->where("`key`='accesstoken:".$this->uniacid."'")->get();
            $data=array("token"=>$tok['access_token'],"expire"=>time());
             
	        if(empty($data0)){
	        	$db->table("core_cache")->add(array("key"=>"accesstoken:".$this->uniacid,"value"=>serialize($data)));
	        }else{
	        	$db->table("core_cache")->where("`key`='accesstoken:".$this->uniacid."'")->save(array("value"=>serialize($data)));
	        } 
    ...
    public function getCacheToken(){
        ...
        $data=unserialize($data0['value']); 
        
        if((intval($data['expire'])<(time()-7150)||strlen($data['token'])<100)){//过期刷新
