...
//			$bj_id = pdo_fetch("SELECT * FROM " . tablename($this->table_classify) . " where weid = :weid AND tid = :tid AND type = :type ", array(':weid' => $_W ['uniacid'], ':tid' => $teachers['id'], ':type' => 'theclass'));
		    $leave = pdo_fetchall("SELECT * FROM " . tablename($this->table_leave) . " where :schoolid = schoolid And :weid = weid And :tid = tid And :isliuyan = isliuyan ORDER BY createtime DESC", array(
		         ':weid' => $weid,
				 ':schoolid' => $schoolid,
//				 ':bj_id' => $bj_id['sid'],
				 ':tid' => $teachers['id'],
