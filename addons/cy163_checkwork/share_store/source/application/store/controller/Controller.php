...
class Controller extends \think\Controller
{
    ...
    {
        // 商家登录信息
        $this->store = Session::get('share_store');
        // 当前路由信息
        $this->getRouteinfo();
        // 验证登录
        $this->checkLogin();
    ...
    {
        require_once(__DIR__ . '/../../../../../../framework/version.inc.php');
        ...
        if (!in_array($this->routeUri, $this->notLayoutAction)) {
            ...
            $this->assign([
                ...
                'we7_url' => $this->getWe7Url(),               // 微擎url
                'we7_copyright' => (new Setting)->getWqSetting('copyright'),
                'we7_version' => IMS_VERSION
            ]);
        }
    }
     ...
     * @return bool|string
     */
    private function getWe7Url()
    {
        $current_url = $this->request->baseUrl(true);
        return substr($current_url, 0, stripos($current_url, 'addons/'));
    }
