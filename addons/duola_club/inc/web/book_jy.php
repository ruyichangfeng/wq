...
if ($operation == 'display') {
    ...
    $book_jy = pdo_fetchall("SELECT * FROM " . tablename($this->table_bookmargin) . " WHERE type = '{$modelName}'  ORDER BY id ASC");
} elseif ($operation == 'post') {
    $parentid = intval($_GPC['parentid']);
    $id = intval($_GPC['id']);
    if (!empty($id)) {
        $book_jy = pdo_fetch("SELECT * FROM " . tablename($this->table_bookmargin) . " WHERE id = '$id'");
    } else {
        $book_jy = array(
            'ssort' => 0,
        );
