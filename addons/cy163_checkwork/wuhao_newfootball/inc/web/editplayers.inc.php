...
if(checksubmit('submit')){
	...
	$result=pdo_update('newfootball_players', $data, array('id' => $data['id'],'uniacid'=>$data['uniacid']));
	if($result){
		message('更新球员成功',$this->createWebUrl('listallplayers',array()),'success');
	}else{
		message('更新球员失败',$this->createWebUrl('listallplayers',array()),'error');
	}
}else{
	$players=pdo_fetch("SELECT * FROM ".tablename('newfootball_players')." WHERE `uniacid`=:uniacid AND `id`=:id",array(':uniacid'=>$_W['uniacid'],':id'=>$_GPC['id']));
