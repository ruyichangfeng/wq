...
if($op == 'index') { 
		...
		if(!isset($_GET['code'])){
			...
			$aconfig = pdo_get('xc_config',$wheres); 
			$money = $aconfig['vip_price'];
			if(!empty($openid)){	
				...
				$money = $_GPC['money']; 			  	
					...
					$url =  $http."://".$_SERVER['HTTP_HOST']."/payment/jsapi/vpay.php?id=".base64_encode($str)."";
					header("location:$url");
	
}
include $this->template('upvip');
