...
//		$bzrtid3 = $category[$teachers['bj_id3']]['tid'];
        ...
        if(!empty($userid['id'])){
		    ...
		    $leave1 = pdo_fetchall("SELECT * FROM " . tablename($this->table_notice) . " where :schoolid = schoolid And :weid = weid And :tid = tid  And :type = type ORDER BY createtime DESC", array(
		         ':weid' => $weid,
				 ':schoolid' => $schoolid,
