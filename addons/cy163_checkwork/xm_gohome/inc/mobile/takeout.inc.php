...
$foo = $_GPC['foo'];
$foos = array('index', 'getIndex', 'getTop', 'list', 'getList', 'page', 'pageinfo', 'setorder', 'setaddress', 'setaddressok', 'ordercar', 'orderpay', 'rate', 'getRate', 'info', 'keep', 'away', 'zan', 'payok');
...
if($foo == 'getTop'){
	...
	if (!empty($_GPC['top1'])) {
		...
		$arr = pdo_fetchall("select * from ".tablename('xm_gohome_merchant')." where weid=".$_W['uniacid']." and stop=1 and state=1 and delstate=1 and top1 !=1 order by id desc, adrstr desc limit ".$startCount.",".$pageSize."");	
	}
	...
	if(empty($arr)){
		...
		foreach($arr as $value){
			...
			$merchant_name 	   = $value['merchant_name'];
			$ordersum      	   = $value['ordersum'];
			$type_name         = $this->getTypeInfo($value['type_id'], 'type_name');
			
			$idStr .= '{"id":"'.$id.'","url":"'.$url.'","icon":"'.$icon.'","merchant_name":"'.$merchant_name.'","merchant_name":"'.$merchant_name.'","ordersum":"'.$ordersum.'","type_name":"'.$type_name.'"},';
		}
		$idStr = rtrim($idStr,',');
		$end = ']';
		$json = $start.$idStr.$end;
...
if($foo == 'getList'){
	...
	if(!empty($xiao)){
		$arr = pdo_fetchall("select * from ".tablename('xm_gohome_merchant')." where weid=".$_W['uniacid']." and stop=1 and state=1 and delstate=1 and adrstr like '".$adrstr."%' ".$condition." order by adrstr desc,ordersum desc,addtime desc limit ".$startCount.",".$pageSize."");
	}else{
		$arr = pdo_fetchall("select * from ".tablename('xm_gohome_merchant')." where weid=".$_W['uniacid']." and stop=1 and state=1 and delstate=1 and adrstr like '".$adrstr."%' ".$condition." order by adrstr desc,addtime desc limit ".$startCount.",".$pageSize."");
	}
	...
	if(empty($arr)){
		...
		foreach($arr as $value){
			...
			$disc		       = $value['disc'];
			$disc_money              = $this->getDiscInfo($id, 'disc');
			$idStr .= '{"id":"'.$id.'","url":"'.$url.'","icon":"'.$icon.'","merchant_name":"'.$merchant_name.'","juli":"'.$juli.'","ordersum":"'.$ordersum.'","merchant_price":"'.$merchant_price.'","merchant_song":"'.$merchant_song.'","xing":"'.$xing.'","new_disc":"'.$new_disc.'","new_disc_money":"'.$new_disc_money.'","man1_disc":"'.$man1_disc.'","man1":"'.$man1.'","jian1":"'.$jian1.'","man2_disc":"'.$man2_disc.'","man2":"'.$man2.'","jian2":"'.$jian2.'","man3_disc":"'.$man3_disc.'","man3":"'.$man3.'","jian3":"'.$jian3.'","disc":"'.$disc.'","disc_money":"'.$disc_money.'"},';
		}
		$idStr = rtrim($idStr,',');
		$end = ']';
		$json = $start.$idStr.$end;
...
if($foo == 'orderpay'){
	...
	if($paytype == 2){
		...
		$this->send_taketmpmsg('ttmpmsg_id', $this->getMerchantInfo($item['merchantid'], 'openid'), $jump, $item['id']);
		...
		if($pay){
			$body = '外卖付款';
			$attach = urlencode("G#".$_W['uniacid']."#".$_W['siteroot']."#".$openid."#".$fee."#".$body."#".$this->generate_code(8)."#".$item['id']);
			$jump   = urlencode($_W['siteroot'].'app/index.php?i='.$_W['uniacid'].'&c=entry&do=takeout&foo=payok&m=xm_gohome');
