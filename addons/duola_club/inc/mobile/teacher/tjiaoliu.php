 ...
 */
	   ...
	   if(!empty($userid['id'])){
            ...
            $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_leave) . " WHERE weid = :weid AND schoolid =:schoolid AND tid = :tid AND leaveid = :leaveid And :isliuyan = isliuyan ORDER BY createtime DESC", array(
					':weid' => $_W['uniacid'],
					':schoolid' => $schoolid,
					':tid' => $leave['tid'],
					':leaveid' => $lid,
