...
//读取区域信息
$article = "select *  from ".tablename('xc_area')." where uniacid=".$_W['uniacid']." order by id desc";
		...
		foreach($list as $k=>$v){		
			...
			//$agent_info = pdo_get("SELECT * FROM ".tablename('xc_agent')." where id=".$v['aid']);
...
include $this->template('list_area');
