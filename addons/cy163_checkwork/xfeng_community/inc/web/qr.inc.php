...
if($op == 'list') {
	...
	include $this->template('web/qr/list');
}
...
if($op == 'del') {
	if ($_GPC['scgq']) {
		...
		message('执行成功<br />删除二维码：'.count($list), referer(),'success');
	}else{
		$id = $_GPC['id'];
...
if($op == 'add') {
	...
	// }else{
		$dps = pdo_fetchall("SELECT * FROM".tablename('xcommunity_dp')."WHERE weid=:uniacid",array(':uniacid' => $_W['uniacid']));
	// }
	...
	if(checksubmit('submit')){
		...
		if (!empty($id)) {
			...
			message('恭喜，更新带参数二维码成功！', $this->createWebUrl('qr',array('op' => 'list')), 'success');
		}
		...
		if(!is_error($result)) {
			...
			message('恭喜，生成带参数二维码成功！', $this->createWebUrl('qr',array('op' => 'list')), 'success');
		} else {
	...
	include $this->template('web/qr/add');
}
...
if($op == 'order') {
	...
	$pager = pagination($total, $pindex, $psize);
	include $this->template('web/qr/order');
}
