...
class Goods extends Base
{
    ...
    public function getImage()
    {
        ...
        $goods_images_url = [];
        if (count($this->goods['image']) > 1) {
            foreach ($this->goods['image'] as $k => $val) {
                $k > 0 && $goods_images_url[] = $this->saveTempImage($wxappId, $val['file_path'], 'goods');
            }
        }
        // 小程序码参数
        ...
        // 拼接海报图
        return $this->savePoster($backdrop, $goodsUrl, $qrcode, $goods_images_url);
    }
     ...
     * @throws \Exception
     */
    private function savePoster($backdrop, $goodsUrl, $qrcode, $goods_images_url = [])
    {
        // 实例化图像编辑器
        ...
        if ($goods_images_url) {
            $num = count($goods_images_url) + 1;
            $goods_images = array_merge([$goodsUrl],$goods_images_url);
            if ($num >= 4) {
                // 打开商品图片
                $editor->open($goodsImage, $goods_images[0]);
                // 重设商品图片宽高
                $editor->resizeExact($goodsImage, 340, 340);
                // 商品图片添加到背景图
                $editor->blend($backdropImage, $goodsImage, 'normal', 1.0, 'top-left', 30, 30);
                // 打开商品图片
                $editor->open($goodsImage, $goods_images[1]);
                // 重设商品图片宽高
                $editor->resizeExact($goodsImage, 340, 340);
                // 商品图片添加到背景图
                $editor->blend($backdropImage, $goodsImage, 'normal', 1.0, 'top-left', 380, 30);

                // 打开商品图片
                $editor->open($goodsImage, $goods_images[2]);
                // 重设商品图片宽高
                $editor->resizeExact($goodsImage, 340, 340);
                // 商品图片添加到背景图
                $editor->blend($backdropImage, $goodsImage, 'normal', 1.0, 'top-left', 30, 380);
                // 打开商品图片
                $editor->open($goodsImage, $goods_images[3]);
                // 重设商品图片宽高
                $editor->resizeExact($goodsImage, 340, 340);
                // 商品图片添加到背景图
                $editor->blend($backdropImage, $goodsImage, 'normal', 1.0, 'top-left', 380, 380);
            } else {
                // 打开商品图片
                $editor->open($goodsImage, $goods_images[0]);
                // 重设商品图片宽高
                $editor->resizeExact($goodsImage, 340, 340);
                // 商品图片添加到背景图
                $editor->blend($backdropImage, $goodsImage, 'normal', 1.0, 'top-left', 200, 30);
                // 打开商品图片
                $editor->open($goodsImage, $goods_images[1]);
                // 重设商品图片宽高
                $editor->resizeExact($goodsImage, 340, 340);
                // 商品图片添加到背景图
                $editor->blend($backdropImage, $goodsImage, 'normal', 1.0, 'top-left', 200, 380);
            }
        } else {
             // 打开商品图片
        ...
        $selling_point = $this->wrapText($fontSize, 0, $fontPath, $this->goods['selling_point'], 520, 3);
        $desc = $this->wrapText(15, 0, $fontPath, $this->goods['goods_sales'].'位好友品质助力/请放心购', 680, 2);
        // 写入商品名称
        $editor->text($backdropImage, $goodsName, $fontSize, 30, 750, new Color('#333333'), $fontPath);
        // 写入商品价格
        $priceType = [10 => 'goods_price', 20 => 'sharing_price'];
        $editor->text($backdropImage, $this->goods['spec'][0][$priceType[$this->goodsType]], 38, 500, 750, new Color('#ff4444'));


        $editor->text($backdropImage, $desc, 15, 30, 810, new Color('#333333'), $fontPath);
        //$editor->text($backdropImage, $sdesc, $fontSize, 50, 810, new Color('#333333'), $fontPath);
        //TODO

         // 写入卖点
        $editor->text($backdropImage, $selling_point, $fontSize, 42, 934, new Color('#333333'), $fontPath);
        
        //$editor->text($backdropImage, $this->goods['spec'][0][$priceType[$this->goodsType]], 38, 62, 964, new Color('#ff4444'));
          // 打开小程序码4
