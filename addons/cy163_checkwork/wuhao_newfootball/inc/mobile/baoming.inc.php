...
if($baomingstatus){
	$data_players=pdo_fetch("SELECT * FROM ".tablename('newfootball_players')." WHERE `openid`=:openid AND `uniacid`=:uniacid",array(':openid'=>$_W['fans']['openid'],':uniacid'=>$_W['uniacid']));	
	...
	if($data_players){
		if($data_players['img'] !== $_W['fans']['headimgurl']) {
            ...
            $result=pdo_update('newfootball_players', $data_players, array('openid' => $_W['fans']['openid'],'uniacid'=>$_W['uniacid']));            
            if($result){                 
	...
	$data_groups_baomingnum=pdo_fetch("SELECT * FROM ".tablename('newfootball_groups')." WHERE `uniacid`=:uniacid AND `groupname`=:groupname",array(':uniacid'=>$_W['uniacid'],':groupname'=>$data_players['groupname']));
	...
	foreach($data_groups as $index_groups=>$item_groups){	
		...
		$data=pdo_fetch("SELECT * FROM ".tablename('newfootball_baoming')." WHERE `matchid`=:matchid AND `id`=:id AND `uniacid`=:uniacid",array(':matchid'=>$data_baoming['matchid'],':id'=>$data_baoming['id'],':uniacid'=>$data_baoming['uniacid']));    
	    if($data){
	        if($data['paystatus'] == 1) {
	            if($data['baomingstatus'] != $data_baoming['baomingstatus']) {		             
	                $result=pdo_update('newfootball_baoming', array('baomingstatus' => $data_baoming['baomingstatus'],'create_time'=>$data_baoming['create_time']), array('matchid' => $data_baoming['matchid'],'id'=>$data_baoming['id'],'uniacid'=>$data_baoming['uniacid']));
	            ...
	            if($data_baoming['baomingstatus'] == 'baoming'){  
	                ...
	                if($data['baomingstatus'] != $data_baoming['baomingstatus']) {		                    
	                    $result=pdo_update('newfootball_baoming', array('baomingstatus' => $data_baoming['baomingstatus']), array('matchid' => $data_baoming['matchid'],'id'=>$data_baoming['id'],'uniacid'=>$data_baoming['uniacid']));
	                    if($result){		                         
	                         if($data_baoming['baomingstatus'] == "请假"){				                	
	                            echo '<script type="text/javascript">alert("更改状态为请假成功！");</script>';
			            	}
	        ...
	        if($data_baoming['baomingstatus'] == 'baoming'){
	        	if($data_groups_baomingnum['expect_baomingnum'] <= $havebaomingnum){
		            ...
		            if($data_baoming['price']>0){
		                ...
		                $result=pdo_insert('newfootball_baoming',$data_baoming);
		                if($result){
