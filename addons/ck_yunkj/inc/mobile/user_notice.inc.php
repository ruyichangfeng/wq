...
$op = $_GPC['op'];
$urltk = $this->createMobileUrl('user_notice');

//修改为已阅读----
$list_wyd = pdo_fetchall("SELECT * FROM ".tablename('cwgl_notice')." WHERE weid = '{$_W['uniacid']}' and uid = '{$_W['member']['uid']}' and readt = '0' and type = 'system' ORDER BY dateline DESC, id DESC");
if (!empty($list_wyd)) {
...
$total = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('cwgl_notice')." WHERE weid = '{$_W['uniacid']}' and uid = '{$_W['member']['uid']}' and type = 'system' {$where}");
if($total){
	$list = pdo_fetchall("SELECT * FROM ".tablename('cwgl_notice')." WHERE weid = '{$_W['uniacid']}' and uid = '{$_W['member']['uid']}' and type = 'system' {$where} {$ordersql} LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
}
