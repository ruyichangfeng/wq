...
date_default_timezone_set('PRC'); 	
   ...
   $ac = new Actions();
	...
	if(!isset($_GET['code'])){
		//当前公众号信息
		$redirect_uri = urlencode('http://'.$_SERVER['SERVER_NAME'].'/addons/mx_smartbox/agent/qrcode.php');
		...
		$code = $_GET['code'];				
			...
			//2、查询设备系统配置api通信信息，判断设备是否在线
				...
				$redirect =  $http."://".$_SERVER['SERVER_NAME'].'/app/index.php?i='.(int)$uniacid.'&aid='.(int)$aid.'&c=entry&do=mindex&m=mx_smartbox';
			...
			//3、查询用户账户信息
				...
				if($user_vip == 1){ //会员身份判断 ： VIP用户
					if($user_money >= $device_rs['vip_price']){ //VIP 账户余额足够，扣除余额结算			
						...
						$repay_url = $http."://".$_SERVER['HTTP_HOST']."/addons/mx_smartbox/agent/pay.php?id=".base64_encode($str)."";
