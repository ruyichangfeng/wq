...
//		    $bj_id = pdo_fetch("SELECT * FROM " . tablename($this->table_classify) . " where weid = :weid AND tid = :tid And type = :type", array(':weid' => $_W ['uniacid'] , ':tid' => $it['tid'], ':type' => 'theclass'));
		    $leave = pdo_fetchall("SELECT * FROM " . tablename($this->table_leave) . " where :schoolid = schoolid And :weid = weid And :tid = tid And :isliuyan = isliuyan ORDER BY createtime DESC LIMIT 0,50", array(
		         ':weid' => $weid,
				 ':schoolid' => $schoolid,
				 ':tid' => $teachers['id'],
				 ':isliuyan' => 1
