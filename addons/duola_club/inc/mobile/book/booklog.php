...
//if(!empty($user)){
    if($op == 'display'){
        ...
        //查询已完成订单
        $borrowOrder = pdo_fetchall("SELECT bookIds from " .tablename($this->table_myorder). "where bookownerid = '{$openid}' and orderStatus = 4");//and orderStatus = 3
        $transferOrder = pdo_fetchall("SELECT outBookIds from " .tablename($this->table_mytransferorder). "where openid = '{$openid}' and orderStatus = 6");//and orderStatus = 5
        $bIds = array();
        ...
        $links = $_W['siteroot'] .'app/'.$this->createMobileUrl('booklog',array('schoolid' => $schoolid,'openid' => $openid));
