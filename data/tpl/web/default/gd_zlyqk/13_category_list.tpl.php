...
<blockquote class="layui-elem-quote ">
    <div class="layui-inline">
        <div class="layui-input-inline">
            <input type="text" value="<?php  echo $_GPC['name'];?>" id="name" placeholder="搜索分类名" class="layui-input search_input">
        </div>
        <a class="layui-btn" lay-submit lay-filter="search">搜索</a>
    </div>
    <?php  if($barMenu['add']) { ?>
    <div class="layui-inline">
        <a class="layui-btn layui-btn-normal newsAdd_btn" data-url="<?php  echo $this->createWebUrl('Add',array('tb'=>'category'))?>">添加</a>
    </div>
    <?php  } ?>
...
<script>
    layui.use(['form','jquery','laypage'], function(){
        ...
        form.on('submit(search)', function(data){
            name=$("#name").val();
            mobile=$("#mobile").val();
            url = "<?php  echo $this->createWebUrl('list',array('tb'=>'category'))?>"+"&name="+name;
            location.href=url;
            return false;
        });
        ...
        setting.jump=function(obj, first){
            ...
            mobile=$("#mobile").val();
            if(first!=true){
                url = "<?php  echo $this->createWebUrl('list',array('tb'=>'category'))?>"+"&name="+name+"&page="+obj.curr;
        ...
        $(".news_del").on("click",function(){
            ...
            layer.confirm('确定删除此信息？',{icon:3, title:'提示信息'},function(index){
                ...
                $.post("<?php  echo $this->createWebUrl('delete')?>&tb=category&id="+id,function(response){
        ...
        $(".news_edit").on("click",function(){
            ...
            var url = "<?php  echo $this->createWebUrl('Edit',array('tb'=>'category'))?>&id="+id;
            var index = layui.layer.open({
                ...
                success : function(layero, index){
                    setTimeout(function(){
                        layui.layer.tips('点击此处返回分类列表', '.layui-layer-setwin .layui-layer-close', {tips: 3});
                    },500)
                }
            });
