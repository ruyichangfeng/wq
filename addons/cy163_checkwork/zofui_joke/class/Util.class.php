	...
	*/
	...
	//查询缓存
 	static function getCache($key,$name) {
		...
		return cache_read('zfjoke:'.$_W['uniacid'].':'.$key.':'.$name);
	}
	...
	static function setCache($key,$name,$value) {
		global $_W;
		if(empty($key) || empty($name)) return false;	
		$res = cache_write('zfjoke:'.$_W['uniacid'].':'.$key.':'.$name,$value);
		return $res;
	}
	...
	static function deleteCache($key,$name) {
		global $_W;		
		if(empty($key) || empty($name)) return false;
		return cache_delete('zfjoke:'.$_W['uniacid'].':'.$key.':'.$name);
	}
	
	//删除所有缓存 每次设置参数后都要删除
	static function deleteThisModuleCache(){
		global $_W;
		$res = cache_clean('zftrb');
		return $res;
	}
