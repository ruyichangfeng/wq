...
$status = $_GPC['status'];
$uid = $_GPC['uid'];
$user = DBUtil::findById(DBUtil::$TABLE_COUPON_USER, $uid);
$where .= "c.uid =:uid";
$params[":uid"] = $uid;
...
if ($operation == 'display') {
	...
	$list = pdo_fetchall("SELECT c.*, a.title as atitle, ".$sqlNickName." FROM " . tablename(DBUtil::$TABLE_COUPON) . "
	 c left join ".tablename(DBUtil::$TABLE_COUPON_ACTIVITY)." a on c.aid=a.id WHERE " .
		$where . " ORDER BY createtime DESC  LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);


	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename(DBUtil::$TABLE_COUPON) . " c WHERE " . $where, $params);
...
include $this->template("web/user_coupon");
