...
<blockquote class="layui-elem-quote ">
    <div class="layui-inline">
        <div class="layui-input-inline">
            <input type="text" value="<?php  echo $_GPC['title'];?>" id="name"  placeholder="标题搜索" class="layui-input search_input">
        </div>
        <a class="layui-btn" lay-submit lay-filter="search">搜索</a>
    </div>
    <?php  if($barMenu['add']) { ?>
    <div class="layui-inline">
        <a class="layui-btn layui-btn-normal newsAdd_btn" data-url="<?php  echo $this->createWebUrl('Add',array('tb'=>'article'))?>">添加</a>
    </div>
    <?php  } ?>
...
<div class="layui-form news_list">
    <table class="layui-table">
        ...
        <tr>
            ...
            <td align="left"><?php  echo $admin['title'];?></td>
            <td align="left"><?php  echo $admin['public'];?></td>
            <td><?php  if($admin['is_admin']==0) { ?>所有人<?php  } else if($admin['is_admin']==1) { ?>客户<?php  } else { ?>员工<?php  } ?></td>
            <td><?php  echo date("Y-m-d",$admin['create_time'])?></td>
            <td><?php  if($admin['gone_time']) { ?><?php  echo date("Y-m-d",$admin['gone_time'])?><?php  } ?></td>
            <td>
...
<script>
    layui.use(['form','jquery','laypage'], function(){
        ...
        form.on('submit(search)', function(data){
            name=$("#name").val();
            url = "<?php  echo $this->createWebUrl('list',array('tb'=>'article'))?>"+"&title="+name;
            location.href=url;
            return false;
        });
        ...
        setting.jump=function(obj, first){
            ...
            mobile=$("#mobile").val();
            if(first!=true){
                url = "<?php  echo $this->createWebUrl('list',array('tb'=>'article'))?>"+"&mobile="+mobile+"&name="+name+"&page="+obj.curr;
        ...
        $(".news_del").on("click",function(){
            ...
            layer.confirm('确定删除此信息？',{icon:3, title:'提示信息'},function(index){
                ...
                $.post("<?php  echo $this->createWebUrl('delete')?>&tb=article&id="+id,function(response){
        ...
        $(".news_edit").on("click",function(){
            ...
            var url = "<?php  echo $this->createWebUrl('Edit',array('tb'=>'article'))?>&id="+id;
