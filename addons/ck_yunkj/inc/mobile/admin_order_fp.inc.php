...
$did = $_GPC['did'];
$urltk = $this->createMobileUrl('admin_order_fp')."&did=".$did;
...
if(checksubmit('listsubmit')){
	...
	if(!empty($_GPC['ygid']) && !empty($_GPC['did'])){
		...
		message('分配成功！', $this->createMobileUrl('admin_order', array('op' => 'view', 'id' => $_GPC['did'])), 'success');
	}else{
		message('分配失败！');
	}
...
include $this->template('admin_order_fp');