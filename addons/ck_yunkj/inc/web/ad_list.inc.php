...
$urlt = $this->createWebUrl('ad_list');
if($kd){
	$urlt .= "&kd=".$kd;
	$where .= " and kd = '{$kd}'";	
} 
...
$list = pdo_fetchall("SELECT * FROM " . tablename('cwgl_ab_list') . " WHERE weid = '{$_W['uniacid']}' AND type = 'ad' {$where} ORDER BY listorder ASC,id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('cwgl_ab_list') . " WHERE weid = '{$_W['uniacid']}' AND type = 'ad' {$where}");
