...
if ($op == 'display') {
    $versionfile = IA_ROOT . '/addons/fk_cs/inc/version.php';
        ...
        if ($ret['result'] == 1) {
            ...
            if (!empty($ret['files'])) {
                foreach ($ret['files'] as $file) {
                    $entry = IA_ROOT . "/addons/fk_cs/" . $file['path'];
                    if (!is_file($entry) || md5_file($entry) != $file['hash']) {
                        ...
                        $files[] = array('path' => $file['path'], 'download' => 0);
                         ...
                         if( is_file($entry) && strexists($entry, 'template/mobile') && strexists($entry, '.html') ){
                              $templatefiles.= "M   /addons/fk_cs/".$file['path']."\r\n";
                         }else{
                            $ft .= "F   /addons/fk_cs/".$file['path']."\r\n";
                         }
                    }
                } 
            } 
    ...
    if (!empty($path)) {
        ...
        if (is_array($ret)) {
            ...
            if (!is_dir(IA_ROOT . "/addons/fk_cs/" . $dirpath)) {
               mkdirs(IA_ROOT . "/addons/fk_cs/" . $dirpath, "0777");
            }

            $content = base64_decode($ret['content']);

            file_put_contents(IA_ROOT . "/addons/fk_cs/" . $path, $content);
               if(isset($ret['path1'])) {
                    $path1 = $ret['path1'];
                    $dirpath1 = dirname($path1);
                    if (!is_dir(IA_ROOT . "/addons/fk_cs/" . $dirpath1)) {
                        mkdirs(IA_ROOT . "/addons/fk_cs/" . $dirpath1, "0777");
                    }
                    $content1 = base64_decode($ret['content1']);
                    file_put_contents(IA_ROOT . "/addons/fk_cs/" . $path1, $content1);
               }
        ...
        if (!empty($upgrade['upgrade'])) {
            $updatefile = IA_ROOT . "/addons/fk_cs/sys/upgrade.php";
            file_put_contents($updatefile, base64_decode($upgrade['upgrade']));
            require $updatefile;
            @unlink($updatefile);
        }
        load()->func('file');
        @rmdirs(IA_ROOT . "/addons/fk_cs/tmp");
        file_put_contents(IA_ROOT . "/addons/fk_cs/inc/version.php", "<?php if(!defined('IN_IA')) {exit('Access Denied');}if(!defined('FM_PHOTOSVOTE_VERSION')) {define('FM_PHOTOSVOTE_VERSION', '" . $upgrade['version'] . "');}if(!defined('FM_PHOTOSVOTE_TIME')) {define('FM_PHOTOSVOTE_TIME', '" . $upgrade['uptime'] . "');}");
    ...
    file_put_contents(IA_ROOT . "/addons/fk_cs/inc/version.php", "<?php if(!defined('IN_IA')) {exit('Access Denied');}if(!defined('FM_PHOTOSVOTE_VERSION')) {define('FM_PHOTOSVOTE_VERSION', '1.0');}");
