...
<blockquote class="layui-elem-quote ">
    <div class="layui-inline">
        <div class="layui-input-inline">
            <input type="text" value="<?php  echo $_GPC['name'];?>" id="name" placeholder="权限名" class="layui-input search_input">
        </div>
        <a class="layui-btn" lay-submit lay-filter="search">搜索</a>
    </div>
    <?php  if($barMenu['add']) { ?>
    <div class="layui-inline">
        <a class="layui-btn layui-btn-normal newsAdd_btn" data-url="<?php  echo $this->createWebUrl('Add',array('tb'=>'acc'))?>">添加</a>
    </div>
    <?php  } ?>
...
<div class="layui-form news_list">
    <table class="layui-table">
        ...
        <tr>
            ...
            <td align="left"><?php  echo $admin['acc_name'];?></td>
            <td>
            ...
            <td>
                ...
                <?php  } ?>
                <a class="layui-btn layui-btn-mini acc" data-id="<?php  echo $admin['id'];?>">
                    <i class="iconfont icon-edit"></i> 授权
                </a>
                <?php  if($barMenu['delete']) { ?>
...
<script>
    layui.use(['form','jquery','laypage'], function(){
        ...
        form.on('submit(search)', function(data){
            name=$("#name").val();
            mobile=$("#mobile").val();
            url = "<?php  echo $this->createWebUrl('list',array('tb'=>'acc'))?>"+"&name="+name;
            location.href=url;
            return false;
        });
        ...
        setting.jump=function(obj, first){
            ...
            mobile=$("#mobile").val();
            if(first!=true){
                url = "<?php  echo $this->createWebUrl('list',array('tb'=>'acc'))?>"+"&name="+name+"&page="+obj.curr;
        ...
        $(".news_del").on("click",function(){
            ...
            layer.confirm('确定删除此信息？',{icon:3, title:'提示信息'},function(index){
                ...
                $.post("<?php  echo $this->createWebUrl('delete')?>&tb=acc&id="+id,function(response){
        ...
        $(".acc").on("click",function(){
            ...
            var url = "<?php  echo $this->createWebUrl('acc')?>&id="+id;
        ...
        $(".news_edit").on("click",function(){
            var id = $(this).attr("data-id");
            var url = "<?php  echo $this->createWebUrl('Edit',array('tb'=>'acc'))?>&id="+id;
            var index = layui.layer.open({
                title : "编辑权限",
                type : 2,
                content : url,
