...
EOF;
  ...
  //是否关注
  function is_follow($fromuser='',$unionid=""){
  	global $_W,$_GPC;
  	$uniacid=$_W['uniacid'];
  	$con=" unionid='$unionid'";
  	$fromuser=$_W['openid'];
  	...
  	$token_code=md5($_W['config']['setting']['authkey']);
  	
  	$gz=$_COOKIE["token_code_$uniacid"];
  	
  	if ($gz){
  	  return true;
  	}
  	
  	if ($_GPC['token_code']==$token_code){
  	  setcookie("token_code_$uniacid","1",time()+3600*24);
  	  return true;
  	}
...
function getNewAddr($location){
  ...
  function add_fans_score($obj){
  	...
  	mc_credit_update($uid,"credit1",$obj['credit1'],array($uid,"最美家乡投票"));
