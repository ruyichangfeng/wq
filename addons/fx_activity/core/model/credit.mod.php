<?php
	...
	function credit_get_by_uid($uid = '' ,$credit_type=1) {
	 ...
	 function credit_update_credit1($uid ,$credit1=0,$credit_type=1,$remark='',$clerk_type=1) {
		global $_W;
		if($credit_type==1){
			load()->model('mc');
			$f =mc_credit_update($uid, 'credit1', $credit1, array($uid, $remark,MODULE_NAME,'5'=>$clerk_type));
			if($f){
				$data=array(
					'uid'=>$uid,
					'uniacid'=>$_W['uniacid'],
