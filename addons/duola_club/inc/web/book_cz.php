...
if ($operation == 'display') {
    ...
    $book_cz = pdo_fetchall("SELECT * FROM " . tablename($this->table_bookmargin) . " WHERE type = '{$modelName}'  ORDER BY id ASC");
} elseif ($operation == 'post') {
    $parentid = intval($_GPC['parentid']);
    $id = intval($_GPC['id']);
    if (!empty($id)) {
        $book_cz = pdo_fetch("SELECT * FROM " . tablename($this->table_bookmargin) . " WHERE id = '$id'");
    } else {
        $book_cz = array(
            'ssort' => 0,
        );
    ...
    if (checksubmit('submit')) {
        ...
        $data = array(
            ...
            'type' => $modelName,
            'couponAmount' => floatval($_GPC['couponAmount']),
            'maxAmount'  => floatval($_GPC['maxAmount']),
        );
