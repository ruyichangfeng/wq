...
//如果是校园通知
    ...
    if(checksubmit('submit')){
        ...
        // 开始 type=3 的 if (问卷写入)
            ...
            if($_GPC['type'] == 1){
                $this->imessage('开始群发班级通知,请勿执行任何操作！', $this->createWebUrl('BjtzMsg', array('notice_id' => $notice_id, 'schoolid' => $schoolid, 'weid' => $weid, 'tname' => $teacher['tname'], 'bj_id' => $bj_id)), 'success');
            }
            if($_GPC['type'] == 2){
                $this->imessage('开始群发校园通知,请勿执行任何操作！！', $this->createWebUrl('XytzMsg', array('notice_id' => $notice_id, 'schoolid' => $schoolid, 'weid' => $weid, 'tname' => $teacher['tname'], 'groupid' => $groupid)), 'success');
            }
            if($_GPC['type'] == 3){
                $this->imessage('开始群发班级作业,请勿执行任何操作！', $this->createWebUrl('ZuoyeMsg', array('notice_id' => $notice_id, 'schoolid' => $schoolid, 'weid' => $weid, 'tname' => $teacher['tname'], 'bj_id' => $bj_id)), 'success');
            }				
        }else{
    ...
    die (json_encode($data));
   	...
   	$itemall = pdo_fetchall("SELECT noticeid FROM " . tablename($this->table_record) . " WHERE schoolid = '{$schoolid}'");	
	$rowcount    = 0;
    foreach($itemall as $key => $row){
            $items = pdo_fetch("SELECT id FROM " . tablename($this->table_notice) . " WHERE id = :id", array(':id' => $row['noticeid']));
            if(empty($items)){
				pdo_delete($this->table_record, array('id' => $row['noticeid']));
                $rowcount++;
                continue;
            }
    }
	$this->imessage("操作成功！共删除{$rowcount}条无效阅读数据!", referer(), 'success');	
