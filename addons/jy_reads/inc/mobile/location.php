...
if (!empty($reply )) {
	if ($reply ['starttime'] > time ()) {
		...
		foreach ($bonus as $key => $item) {
			// 区域限制
			if($item['location']==1){
				if(!empty($item['area_title'])){
					$temp = strstr($item['area_title'],$cityid);
					if(!$temp){
						unset($bonus[$key]);
						continue;
					}
		...
		// 加入获取卡券==================
		$set_uniacid = $_W['uniacid'];
		$sql = "SELECT * FROM ".tablename('uni_settings')." WHERE uniacid='{$set_uniacid}'";
		$unisetting = pdo_fetch($sql);
		...
		// 添加区域限制==========
		if($reply['location']=='1'){
			$temp = strstr($reply['area_title'],$cityid);
			if(empty($temp)){
				$error = '当前地区不支持该活动';
				include $this->template ('no');
				exit();
