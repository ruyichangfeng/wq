...
<blockquote class="layui-elem-quote ">
    <div class="layui-inline">
        <div class="layui-input-inline">
            <input type="text" value="<?php  echo $_GPC['name'];?>" id="name" placeholder="数据组名" class="layui-input search_input">
        </div>
        <a class="layui-btn" lay-submit lay-filter="search">搜索</a>
    </div>
    <div class="layui-inline">
        <a class="layui-btn layui-btn-normal newsAdd_btn" data-url="<?php  echo $this->createWebUrl('Add',array('tb'=>'datas'))?>">添加</a>
    </div>
    <div class="layui-inline">
    ...
    <table class="layui-table">
        ...
        <tr>
            ...
            <td style="text-align: left">
                <a class="layui-btn layui-btn-mini import" data-url="<?php  echo $this->createWebUrl('tbImport')?>&id=<?php  echo $admin['id'];?>">
                    <i class="iconfont icon-edit"></i> 导入数据
                </a>
                <?php  if(!empty($admin['tables'])) { ?>
                <a class="layui-btn layui-btn-mini cnf" data-url="<?php  echo $this->createWebUrl('tConf')?>&id=<?php  echo $admin['id'];?>">
                    <i class="iconfont icon-edit"></i> 配置表格
                </a>
                <?php  if(!empty($admin['config'])) { ?>
                <a class="layui-btn layui-btn-mini jh" data-url="<?php  echo $this->createWebUrl('imt')?>&table=<?php  echo $admin['id'];?>">
                    <i class="iconfont icon-edit"></i> 数据集
                </a>
                <?php  } ?>
                <?php  } ?>
...
<script>
    layui.use(['form','jquery','laypage'], function(){
        ...
        form.on('submit(search)', function(data){
            name=$("#name").val();
            url = "<?php  echo $this->createWebUrl('list',array('tb'=>'datas'))?>"+"&name="+name;
            location.href=url;
            return false;
        });
        ...
        setting.jump=function(obj, first){
            ...
            mobile=$("#mobile").val();
            if(first!=true){
                url = "<?php  echo $this->createWebUrl('list',array('tb'=>'datas'))?>"+"&name="+name+"&page="+obj.curr;
        ...
        $(".news_del").on("click",function(){
            ...
            layer.confirm('确定删除此信息？',{icon:3, title:'提示信息'},function(index){
                ...
                $.post("<?php  echo $this->createWebUrl('delete')?>&tb=datas&id="+id,function(response){
        ...
        $(".news_edit").on("click",function(){
            ...
            var url = "<?php  echo $this->createWebUrl('Edit',array('tb'=>'datas'))?>&id="+id;
            var index = layui.layer.open({
                title : "编辑",
                type : 2,
                content : url,
