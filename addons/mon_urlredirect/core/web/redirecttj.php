...
if ($operation == 'display') {
    ...
    $sql = "SELECT uid, redid, count(*) vcount,".$rname.", ".$url.",".$urlnamne." FROM "
        . tablename(DBUtil::$TABLE_URLREDIRECT_RECORD) . "  r WHERE " . $where .
        "  group by r.uid ORDER BY vcount DESC limit " . ($pindex - 1) * $psize . ',' . $psize;

    $list = pdo_fetchall($sql, $params);
    $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename(DBUtil::$TABLE_URLREDIRECT_RECORD) . " r   WHERE  " . $where . " group by r.uid", $params);
    $pager = pagination($total, $pindex, $psize);

}


include $this->template("web/tj_list");
