...
class Message
{
     ...
     */
    public function payment($order, $order_type = 10)
    {
        // 1. 微信模板消息
        $template = SettingModel::getItem('tplMsg', $order['wxapp_id'])['payment'];
        if ($template['is_enable'] && !empty($template['template_id'])) {
            // 获取 prepay_id
            $prepayId = $this->getPrepayId($order['order_id'], $order_type);
            // 页面链接
            $urls = [
                10 => 'pages/order/detail',
                20 => 'pages/sharing/order/detail/detail',
            ];
            // 发送模板消息
            $status = $this->sendTemplateMessage($order['wxapp_id'], [
                'touser' => $order['user']['open_id'],
                'template_id' => $template['template_id'],
                'page' => $urls[$order_type] . '?order_id=' . $order['order_id'],
     ...
     * @throws \think\exception\DbException
     */
    public function delivery($order, $order_type = 10)
    {
        // 获取 prepay_id
        $prepayId = $this->getPrepayId($order['order_id'], $order_type);
        // 微信模板消息
     ...
     * @throws \think\exception\DbException
     */
    public function refund($refund, $order_no, $order_type = 10)
    {
        // 获取formid
        ...
        // 页面链接
        $urls = [
            10 => 'pages/order/refund/index',
            20 => 'pages/sharing/order/refund/index',
        ];
        // 发送模板消息
        $status = $this->sendTemplateMessage($refund['wxapp_id'], [
            'touser' => $refund['user']['open_id'],
            'template_id' => $template['template_id'],
            'page' => $urls[$order_type],
     ...
     */
    private function getPrepayId($order_id, $order_type = 10)
    {
        return WxappPrepayIdModel::detail($order_id, $order_type);
    }
