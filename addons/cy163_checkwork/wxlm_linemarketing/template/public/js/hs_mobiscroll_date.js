            ...
            if (event('validate', [dw, index, time]) !== false) {
                ...
                $('.dw-ul', dw).each(function (i) {
                    ...
                    if (!(cell.hasClass('dw-sel')) || sc) {
						...
						/*
						if(i == '0' && v == '100' && time == undefined && isInit === false) {
							isInit = true;
							that.scroll(t, '0', '80', 0, undefined);
						}
						else {
							that.scroll(t, i, v, sc ? time : 0.1, sc ? orig : undefined);
						}
        ...
        */
        that.show = function (prevAnim) {
            ...
            var html = '<div class="dw-trans ' + s.theme + ' dw-' + s.display + '">' + (s.display == 'inline' ? '<div class="dw dwbg dwi"><div class="dwwr">' : '<div class="dw-persp">' + '<div class="dwo"></div><div class="dw dwbg ' + mAnim + '"><div class="dw-arrw"><div class="dw-arrw-i"><div class="dw-arr"></div></div></div><div class="dwwr">');
				html += '<div class="dwbc"><span class="dwbw dwb-s"><span class="dwb">' + s.setText + '</span></span></div>';

            for (i = 0; i < s.wheels.length; i++) {
                html += '<div class="dwc' + (s.mode != 'scroller' ? ' dwpm' : ' dwsc') + (s.showLabel ? '' : ' dwhl') + '"><div class="dwwc dwrc"><table style="width:100%;" cellpadding="0" cellspacing="0"><tr>';
                // Create wheels
                for (label in s.wheels[i]) {
                    warr[l] = s.wheels[i][label];
                    html += '<td><div class="dwwl dwrc dwwl' + l + '">' + (s.mode != 'scroller' ? '<div class="dwwb dwwbp" style="height:' + hi + 'px;line-height:' + hi + 'px;"><span>+</span></div><div class="dwwb dwwbm" style="height:' + hi + 'px;line-height:' + hi + 'px;"><span>&ndash;</span></div>' : '') + '<div class="dww" style="height:' + (s.rows * hi) + 'px; width:100%;"><div class="dw-ul">';
                    // Create wheel values
