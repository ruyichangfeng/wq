 ...
 */
	...
	if($op == 'list'){
		...
		if ($_GPC['export'] == 1) {
            $sql = "select id,address,content,rank,category,createtime,status,resolver,dealing,openid,comment,resolve,resolvetime from".tablename('xcommunity_report')."where $condition and type =1 order by id desc ";
...
//                }
        ...
        $list = pdo_fetchall("SELECT * FROM" . tablename('xcommunity_notice') . "WHERE uniacid=:uniacid AND enable = 1 ", array(':uniacid' => $_W['uniacid']));
        $notices ='';
        ...
        if(checksubmit('submit')){
            ...
            sendmessage($item['fansopenid'],$fromuser,$region['title'],$_GPC['rid'],$_GPC['content'],'1');
