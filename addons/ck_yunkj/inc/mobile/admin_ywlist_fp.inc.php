...
$did = $_GPC['did'];
$urltk = $this->createMobileUrl('admin_ywlist_fp')."&did=".$did;
...
if(checksubmit('listsubmit')){
	...
	if(!empty($_GPC['ygid']) && !empty($_GPC['did'])){
		pdo_update('cwgl_service_ywlist', $data, array('id' => $_GPC['did'],'weid' => $_W['uniacid']));
		message('分配成功！', $this->createMobileUrl('admin_ywlist', array('op' => 'view', 'id' => $_GPC['did'])), 'success');
	}else{
		message('分配失败！');
	}
...
include $this->template('admin_ywlist_fp');