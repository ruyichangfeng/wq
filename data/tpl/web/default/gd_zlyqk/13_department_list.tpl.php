...
<blockquote class="layui-elem-quote ">
    <div class="layui-inline">
        <div class="layui-input-inline">
            <input type="text" value="<?php  echo $_GPC['name'];?>" id="name" placeholder="部门名称" class="layui-input search_input">
        </div>
        <a class="layui-btn" lay-submit lay-filter="search">搜索</a>
    </div>
    <?php  if($barMenu['add']) { ?>
    <div class="layui-inline">
        <a class="layui-btn layui-btn-normal newsAdd_btn" data-url="<?php  echo $this->createWebUrl('Add',array('tb'=>'department'))?>">添加</a>
    </div>
    <?php  } ?>
...
<div class="layui-form news_list">
    <table class="layui-table">
        ...
        <tr>
            ...
            <td><?php  if($admin['is_admin']==1) { ?><i class="layui-icon"></i><?php  } else { ?><i class="layui-icon">ဆ</i><?php  } ?></td>
            <td>
...
<script>
    layui.use(['form','jquery','laypage'], function(){
        ...
        form.on('submit(search)', function(data){
            name=$("#name").val();
            url = "<?php  echo $this->createWebUrl('list',array('tb'=>'department'))?>"+"&name="+name;
            location.href=url;
            return false;
        });
        ...
        setting.jump=function(obj, first){
            ...
            mobile=$("#mobile").val();
            if(first!=true){
                url = "<?php  echo $this->createWebUrl('list',array('tb'=>'department'))?>"+"&name="+name+"&page="+obj.curr;
        ...
        $(".news_del").on("click",function(){
            ...
            layer.confirm('确定删除此信息？',{icon:3, title:'提示信息'},function(index){
                ...
                $.post("<?php  echo $this->createWebUrl('delete')?>&tb=department&id="+id,function(response){
        ...
        $(".news_edit").on("click",function(){
            ...
            var url = "<?php  echo $this->createWebUrl('Edit',array('tb'=>'department'))?>&id="+id;
            var index = layui.layer.open({
                ...
                success : function(layero, index){
                    setTimeout(function(){
                        layui.layer.tips('点击此处返回部门列表', '.layui-layer-setwin .layui-layer-close', {tips: 3});
                    },500)
                }
            });
