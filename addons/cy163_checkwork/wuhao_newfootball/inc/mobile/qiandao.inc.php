...
if($qiandaostatus){
	...
	if($data_players){
        ...
        $data_groups_ticketsnum=pdo_fetch("SELECT * FROM ".tablename('newfootball_groups')." WHERE `uniacid`=:uniacid AND `groupname`=:groupname",array(':uniacid'=>$_W['uniacid'],':groupname'=>$data_players['groupname']));
        ...
        $data_baoming=pdo_fetch("SELECT * FROM ".tablename('newfootball_baoming')." WHERE `matchid`=:matchid AND `id`=:id AND `uniacid`=:uniacid",array(':matchid'=>$matchid,':id'=>$data_players['id'],':uniacid'=>$_W['uniacid']));        
        ...
        if($data_baoming['qiandaostatus'] == 'qiandao'){        	
			...
			$data_matches=pdo_fetch("SELECT * FROM ".tablename('newfootball_matches')." WHERE `uniacid`=:uniacid AND `matchid`=:matchid",array(':uniacid'=>$_W['uniacid'],':matchid'=>$matchid));			
