...
//    $is_accept_exchange = $_GPC['is_accept_exchange'] == 2 ? $_GPC['is_accept_exchange'] : 1;
    //查询book
    $result = pdo_fetchall("SELECT mybook.openid,book.* FROM " . tablename($this->table_mybook) . " as mybook left Join ".tablename($this->table_book)." as book on  mybook.bookid = book.id where :schoolid = mybook.schoolid And :weid = mybook.weid  And :is_delete = mybook.is_delete And :status = mybook.status And mybook.openid <> '{$openid}' $condition", array(':weid' => $weid, ':schoolid' => $schoolid,':is_delete' => 1,':status' => 2), 'id');
    if($result){
        $books = array();
        $cats  = array();
        $bookCats = array();
        ...
        foreach($result as $book){
            if(empty($books[$book['openid']])){
                ...
                if(!empty($_GPC['typeName']) && $_GPC['typeName'] == 'distance'){
                    $distance = $book_distance[$_GPC['typeId']];
