...
class Order extends ZskWxapp{
	...
	/*	$orders=$model->where($where)->order(" `date` desc")->page("*,order_no as `num`"); 
		...
		$sql2="SELECT $tb_case.abstract ,$tb_case.isopt,$tb_case.thumb ,temptb.* FROM (SELECT {$tb_detail}.id as did,orderid as `oid`, {$tb_detail}.goodscaseid,{$tb_detail}.goodsname  ,{$tb_detail}.count  ,{$tb_detail}.goodsid  , {$tb_goods}.picture as pic FROM {$tb_detail} LEFT JOIN {$tb_goods} on {$tb_goods}.id = {$tb_detail}.goodsid WHERE {$tb_detail}.orderid in ($idstr) )  as temptb LEFT JOIN $tb_case ON temptb.goodscaseid=$tb_case.id ";
