...
date_default_timezone_set('PRC'); 	
   ...
   $ac = new Actions();
	...
	if(!isset($_GET['code'])){
		//当前公众号信息
		$redirect_uri = urlencode('http://'.$_SERVER['SERVER_NAME'].'/addons/mx_washer/agent/qrcode.php');
			...
			//3、查询用户账户信息
				...
				if($user_vip == 1){ //会员身份判断 ： VIP用户
					if($user_money >= $device_rs['vip_price']){ //VIP 账户余额足够，扣除余额结算			
						...
						$repay_url = $http."://".$_SERVER['HTTP_HOST']."/addons/mx_washer/agent/pay.php?id=".base64_encode($str)."";
