   ...
   $operation = in_array ( $_GPC ['op'], array ('default','addchild','useredit','jiaoliu','bdxs','bdls','unboundls','qingjia','agree','defeid','sagree','sdefeid','savemsg','xsqingjia','savesmsg','getbjlist','signup','txshbm','xgxsinfo','tgsq','jjsq', 'bangdingcardjl', 'jbidcard', 'changeimg', 'changePimg', 'changeimgt','showchecklog') ) ? $_GPC ['op'] : 'default';
	...
	if ($operation == 'bdxs') {
	    ...
	    // $s_name = trim($_GPC['s_name']);
		 ...
		 $userType  = trim($_GPC['userType']) ? trim($_GPC['userType']) : 'parents';
//		$subjectId = trim($_GPC['subjectId']);

		$pid = pdo_fetch("SELECT * FROM " . tablename($this->table_parents) . " where :schoolid = schoolid And :weid = weid And :mobile = mobile And :userType = userType", array(
		         ':weid' => $_GPC ['weid'],
...
//				    );
//			}
            $fans = pdo_fetch("select uid FROM ".tablename('mc_mapping_fans')." WHERE acid = '".$_GPC['weid']."' And openid = '".$_GPC['openid']."' ");
            $temp = array(
				    'schoolid' => $_GPC['schoolid'],
					'weid' => $_GPC['weid'],
					'uid' => $fans['uid'],
