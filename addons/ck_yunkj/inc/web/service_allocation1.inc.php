...
$urlt = $this->createWebUrl('service_allocation1');
...
if(checksubmit('listsubmit')){
	...
	if(!empty($_GPC['ygid']) && !empty($_GPC['did'])){
		...
		message('分配成功！', $this->createWebUrl('user_list', array('op' => 'edit', 'id' => $_GPC['did'])), 'success');
	}else{
		message('分配失败！');
	}
...
include $this->template('service_allocation1');
?>