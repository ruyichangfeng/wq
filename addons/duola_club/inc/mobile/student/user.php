 ...
 */
        ...
        //查询是否用户登录
		$parents = pdo_fetch("SELECT * FROM " . tablename($this->table_parents) . " where weid = :weid AND openid=:openid AND schoolid=:schoolid AND userType = :userType", array(':weid' => $weid, ':openid' => $openid, ':schoolid' => $schoolid,':userType' => 'parents'));
		if(!empty($parents)){
			if(!empty($userss)){
