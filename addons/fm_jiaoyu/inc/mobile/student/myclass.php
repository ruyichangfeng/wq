 ...
 */
		...
		if($it){
			...
			$list = pdo_fetchall("SELECT * FROM " . tablename($this->table_tcourse) . " WHERE weid = :weid AND schoolid =:schoolid AND bj_id = :bj_id ORDER BY end DESC", array(':weid' => $_W['uniacid'], ':schoolid' => $schoolid, ':bj_id' => $student['bj_id']));
