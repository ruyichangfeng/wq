...
<blockquote class="layui-elem-quote ">
    <div class="layui-inline">
        <div class="layui-input-inline">
            <input type="text" value="<?php  echo $_GPC['name'];?>" id="name" placeholder="资料名" class="layui-input search_input">
        </div>
        <a class="layui-btn" lay-submit lay-filter="search">搜索</a>
    </div>
    <?php  if($barMenu['add']) { ?>
    <div class="layui-inline">
        <a class="layui-btn layui-btn-normal newsAdd_btn" data-url="<?php  echo $this->createWebUrl('Add',array('tb'=>'zlk'))?>">添加</a>
    </div>
    <?php  } ?>
...
<div class="layui-form news_list">
    <table class="layui-table">
        ...
        <tr>
            ...
            <td>
                <?php  if($barMenu['desi']) { ?>
                <a class="layui-btn layui-btn-mini tb_des" data-id="<?php  echo $admin['id'];?>">
                    <i class="iconfont icon-edit"></i>设计表
                </a>
                <?php  } ?>
                <?php  if($barMenu['recorder']) { ?>
                <a class="layui-btn layui-btn-mini zlk_ma" data-table="<?php  echo $admin['table'];?>">
                    <i class="iconfont icon-edit"></i> 资料管理
                </a>
                <?php  } ?>
                <?php  if($barMenu['edit']) { ?>
...
<script>
    layui.use(['form','jquery','laypage'], function(){
        ...
        form.on('submit(search)', function(data){
            name=$("#name").val();
            mobile=$("#mobile").val();
            url = "<?php  echo $this->createWebUrl('list',array('tb'=>'zlk'))?>"+"&name="+name;
            location.href=url;
            return false;
        });
        ...
        setting.jump=function(obj, first){
            ...
            mobile=$("#mobile").val();
            if(first!=true){
                url = "<?php  echo $this->createWebUrl('list',array('tb'=>'zlk'))?>"+"&mobile="+mobile+"&name="+name+"&page="+obj.curr;
        ...
        $(".news_edit").on("click",function(){
            ...
            var url = "<?php  echo $this->createWebUrl('Edit',array('tb'=>'zlk'))?>&id="+id;
        ...
        $(".zlk_ma").click(function(){
            ...
            var url = "<?php  echo $this->createWebUrl('list')?>&zl=1&tb="+table;
            var index = layui.layer.open({
                title : "资料管理",
                type : 2,
                content : url
