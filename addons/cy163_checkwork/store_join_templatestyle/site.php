...
class Store_join_templatestyleModuleSite extends WeModuleSite {
	...
	public function doMobileMessage() {
		...
		if (!empty($_GPC['zname']) && !empty($_GPC['zphone']) && !empty($_GPC['zmessage']) && $_GPC['zmessage']!='请在此处输入您的留言 …') {
			...
			$this -> templateMessageReturn($_GPC['zname'], $_GPC['zphone'], $_GPC['zmessage'], $_GPC['reply_url']);
			echo 1;
		} else {
			echo 2;
		}
	...
	public function templateNotice($name, $type, $content,$zmessage,$openid) {
		...
		$access_token = WeAccount::token();
		$url = "https://api.weixin.qq.com/cgi-bin/message/template/send?access_token={$access_token}";
		$data = array('touser' => $openid, 'template_id' => $items['replyid'], 'url' =>$item['news_url'], 'data' => array('first' => array('value' => urlencode("您的反馈与投诉已由客服处理"), 'color' => "#743A3A", ), 'keyword1' => array('value' => urlencode(date('Y-m-d H:i:s', TIMESTAMP)), 'color' => "#0000FF", ), 'keyword2' => array('value' => urlencode($zmessage), 'color' => "#FF0000", ), 'keyword3' => array('value' => urlencode($content), 'color' => "#0000FF", ), 'remark' => array('value' => urlencode($item['reply_color']), 'color' => "#008000", ), ));
		$response = ihttp_request($url, urldecode(json_encode($data)));
	}
