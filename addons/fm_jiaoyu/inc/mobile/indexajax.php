	...
	if ($operation == 'signup')  {
		...
		if (empty($_GPC ['openid']))  {
			...
			if(!empty($_GPC['headimg'])){
				...
				$token2 =  $access_token;				
				$url = 'http://file.api.weixin.qq.com/cgi-bin/media/get?access_token='.$token2.'&media_id='.$_GPC['headimg'];
...
if($typess != 'c'){
	...
	//$JieGuo_temp['question_data'][$key] = $Xuan_temp;
	}
}elseif($typess == 'c')
{
